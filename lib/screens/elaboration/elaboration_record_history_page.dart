...
class ElaborationRecordHistoryPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final box = Hive.box<ElaborationRecord>('elaboration_records');

    return Scaffold(
      appBar: AppBar(title: const Text("Historial de Elaboraciones")),
      body: ValueListenableBuilder(
        valueListenable: box.listenable(),
        builder: (context, Box<ElaborationRecord> box, _) {
          if (box.isEmpty) {
            return const Center(
              child: Text("No existen elaboraciones registradas"),
            );
          }
